  ...
  doLint(projectPath, filePath) {
    return new Promise((resolve, reject) => {
      ...
      const [command, args] = this.buildCommand(
        "perl",
        [
          path.join(__dirname, "../script/lint.pl"),
          path.join(projectPath, atom.config.get(`${pkg.name}.perlLintConfigFileBasename`)),
          filePath
        ]
      );
      const options = {cwd: path.dirname(filePath)};
      function stdout(output) { outBuf.push(output); allBuf.push(output); }
      function stderr(output) { allBuf.push(output); }
      function exit(code) {
        if (!code) {
          resolve(JSON.parse(outBuf.join("")));
        } else {
          reject(new Error(allBuf.join("")));
        }
      }
